...
namespace Material_Evaluation.EmetServices.CompledRequest
{
    /// <summary>
    ...
    public class MyJson : System.Web.Services.WebService
    {
            ...
            , string VendorCode)
        {
            ...
            try
            {
                ...
                sql += @" and (RIGHT(QuoteNo, 2) <> 'GP') and (RIGHT(QuoteNo, 1) <> 'D') ";
                sql += @" and ( RequestNumber not in(select RequestNumber from TQuoteDetails where ManagerApprovalStatus =0 ) and(DIRApprovalStatus=0 or DIRApprovalStatus=1) ) ";
                #region condition if from vendor page
                if (VendorCode.Trim() != "")
                {
                ...
                {
                    sql = @" select distinct Plant,RequestNumber,NoQuote,RequestDate,QuoteResponseDueDate,Product,Material,MaterialDesc,CreatedBy,UseDep,ReqType from ##temp1";
                    cmd = new SqlCommand(sql, EmetCon, EmetTrans);
