...
{
    public partial class EPICEmail : System.Web.UI.Page
    {
        string sql;
        SqlCommand cmd;
        ...
        protected void ShowTable()
        {
            ...
            try
            {
                ...
                {
                    sql = @" select distinct A.VendorCode,B.Description,Approver,ApproverEmail,PICName,PICEmail
                            from TVendorPic A 
                            inner join tVendor_New B on A.VendorCode = B.Vendor 
                            where A.delflag=0 and A.plant = @plant ";
                    if (txtFind.Text != "")
                    {
                        if (DdlFilterBy.SelectedValue.ToString() == "Approver")
                        {
                            sql += @" and A.Approver like '%'+@Filter+'%' ";
                        }
                        else if (DdlFilterBy.SelectedValue.ToString() == "ApproverEmail")
                        {
                            sql += @" and A.ApproverEmail like '%'+@Filter+'%' ";
                        }
                        else if (DdlFilterBy.SelectedValue.ToString() == "PICName")
                        {
                            sql += @" and A.PICName like '%'+@Filter+'%' ";
                        }
                        else if (DdlFilterBy.SelectedValue.ToString() == "PICEmail")
                        {
                            sql += @" and A.PICEmail like '%'+@Filter+'%' ";
                        }
                        else if (DdlFilterBy.SelectedValue.ToString() == "VendorCode")
                        {
                            sql += @" and A.VendorCode like '%'+@Filter+'%' ";
                        }
                        else if (DdlFilterBy.SelectedValue.ToString() == "Description")
                        {
                            sql += @" and B.Description like '%'+@Filter+'%' ";
                        }
                    ...
                    using (DataTable dt = new DataTable())
                    {
                        ...
                        Session["ShowEntryVndPICEmail"] = ShowEntry.ToString();
                        GridView1.DataBind();
                        if (dt.Rows.Count > 0)
                        {
