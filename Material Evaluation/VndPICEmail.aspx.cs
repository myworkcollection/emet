...
{
    public partial class VndPICEmail : System.Web.UI.Page
    {
        string sql;
        SqlCommand cmd;
                ...
                else
                {
                    ...
                    if (!IsPostBack)
                    {
                        ...
                        else
                        {
                            ...
                            if (Session["ShowEntryVndPICEmail"] != null)
                            {
                                TxtShowEntry.Text = Session["ShowEntryVndPICEmail"].ToString();
                            }
                            ShowTable();
                        }
                    }
        ...
        protected void ShowTable()
        {
            ...
            try
            {
                ...
                {
                    sql = @" select Approver,ApproverEmail,PICName,PICEmail
                              from TVendorPic where delflag=0 and plant = @plant and VendorCode=@VendorCode ";
                    if (txtFind.Text != "")
                    {
                        if (DdlFilterBy.SelectedValue.ToString() == "Approver")
                        {
                            sql += @" and Approver like '%'+@Filter+'%' ";
                        }
                        else if (DdlFilterBy.SelectedValue.ToString() == "ApproverEmail")
                        {
                            sql += @" and ApproverEmail like '%'+@Filter+'%' ";
                        }
                        else if (DdlFilterBy.SelectedValue.ToString() == "PICName")
                        {
                            sql += @" and PICName like '%'+@Filter+'%' ";
                        }
                        else if (DdlFilterBy.SelectedValue.ToString() == "PICEmail")
                        {
                            sql += @" and PICEmail like '%'+@Filter+'%' ";
                        }
