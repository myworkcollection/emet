...
{
    public partial class EProcGrpVsSubProc : System.Web.UI.Page
    {
        string sql;
        SqlCommand cmd;
        ...
        protected void ShowTable()
        {
            ...
            try
            {
                ...
                {
                    sql = @" select ProcessGrpCode,ProcessGrpDescription,SubProcessCode,SubProcessName,ProcessUOM,ProcessUomDescription
                              from TPROCESGROUP_SUBPROCESS where delflag=0  ";
                    if (txtFind.Text != "")
                    {
                        if (DdlFilterBy.SelectedValue.ToString() == "ProcessGrpCode")
                        {
                            sql += @" and ProcessGrpCode like '%'+@Filter+'%' ";
                        }
